...
const HighlightedPre = React.memo(async ({ children, language, ...props }: HighlightedPreProps) => {
...
function childrenTakeAllStringContents(element: React.ReactNode): string {
  ...
  if (React.isValidElement(element)) {
    const elementProps = element.props as React.PropsWithChildren<any>; // Typecast props safely
...
const COMPONENTS: { [key: string]: FC<any> } = {
  ...
  code: ({ children, className, ...rest }: { children: React.ReactNode; className?: string }) => {
    ...
    return match ? (
      ...
      <code className={`bg-background/50 px-1 py-0.5 font-mono ${className || ""}`} {...rest}>
        {children}
      </code>
    );
  },
  ...
  th: withClass("th", "border border-foreground/20 px-4 py-2 text-left font-bold"),
...
function withClass<Tag extends keyof JSX.IntrinsicElements>(
  ...
  const Component: FC<JSX.IntrinsicElements[Tag]> = ({ className, ...props }) => {
